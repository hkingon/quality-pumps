...
export default function PipeLibraryPage() {
  ...
  const handleDeleteSize = async (sizeId: string, sizeCreatedBy: string | null) => {
    if (!isAdmin && sizeCreatedBy !== user?.id) {
      toast.error('You can only delete your own pipe sizes');
      return;
    }
  ...
  const handleUpdateSize = async (sizeId: string, sizeCreatedBy: string | null) => {
    if (!isAdmin && sizeCreatedBy !== user?.id) return;
    try {
  ...
  const handleAddSize = async (typeId: string, typeCreatedBy: string | null) => {
  ...
  const canManageType = (type: PipeTypeWithSizes) => isAdmin || type.created_by === user?.id;
  const canManageSize = (size: PipeSize) => isAdmin || size.created_by === user?.id;

  const globalTypes = filteredTypes.filter((t) => !t.created_by);
  const myTypes = filteredTypes.filter((t) => t.created_by === user?.id);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
        <TabsList className='grid w-full grid-cols-2'>
          <TabsTrigger value='global' className='flex items-center gap-2'>
            ...
            Global Types ({globalTypes.reduce((sum, t) => sum + t.sizes.length, 0)})
          </TabsTrigger>
          <TabsTrigger value='my' className='flex items-center gap-2'>
            <User className='h-4 w-4' />
            My Types ({myTypes.reduce((sum, t) => sum + t.sizes.length, 0)})
          </TabsTrigger>
        </TabsList>
  ...
  function renderTypeList(types: PipeTypeWithSizes[], showActions: boolean) {
    return (
      <Card>
        <CardHeader>
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-muted-foreground mt-1 text-sm'>
                {showActions
                  ? 'Manage your custom pipe types and sizes.'
                  : 'Browse global pipe sizes shared by all users.'}
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className='py-8 text-center'>
              <p className='text-muted-foreground'>
                {showActions
                  ? "You haven't created any custom pipe types yet. Go to Manage Pipe Types to create one."
                  : 'No global pipe types available.'}
              </p>
            </div>
          ) : (
              ...
              {types.map((type) => (
                <div key={type.id}>
                  <div className='mb-2 flex items-center gap-2'>
                    ...
                    {showActions && canManageType(type) && (
                      <Button
                        variant='ghost'
                        size='sm'
                        onClick={() => {
                    ...
                    <TableBody>
                      {type.sizes.map((size) => (
                        <TableRow key={size.id}>
                          ...
                          {showActions && canManageSize(size) && (
                            <TableCell className='text-right'>
                              {editSizeId === size.id ? (
                                <div className='flex justify-end gap-2'>
                                  <Button size='sm' onClick={() => handleUpdateSize(size.id, size.created_by)}>
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction
                                          onClick={() => handleDeleteSize(size.id, size.created_by)}
