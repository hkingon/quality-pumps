...
import { GuestSignupDialog } from '@/components/auth/guest-signup-dialog';
...
export function PumpCurveDashboard() {
  ...
  const [showGuestSignupDialog, setShowGuestSignupDialog] = useState(false);
  ...
  // Generate and download a sample example PDF for guest users
  const downloadExamplePDF = () => {
    const pdf = new jsPDF();
    pdf.setFontSize(18);
    pdf.text('Pump Curve Analysis Report — Example', 20, 20);
    pdf.setFontSize(11);
    pdf.text('This is an example report generated by Quality Pumps.', 20, 35);
    pdf.text('Sign up for a free account to generate reports for your own data.', 20, 43);

    pdf.setFontSize(13);
    pdf.text('Example Pump: Grundfos CM5-4', 20, 60);
    pdf.setFontSize(11);
    pdf.text('Max Head: 42.00 m', 20, 70);
    pdf.text('Max Flow: 120.00 L/min', 20, 78);
    pdf.text('Operating Point: 95.0 L/min @ 28.5 m', 20, 86);
    pdf.text('Absorbed Power: 1.42 kW', 20, 94);
    pdf.text('BEP: 87%', 20, 102);

    pdf.setFontSize(13);
    pdf.text('System Curve', 20, 118);
    pdf.setFontSize(11);
    pdf.text('Static Head: 18.0 m', 20, 128);
    pdf.text('Operating Flow: 95.0 L/min', 20, 136);

    pdf.setFontSize(9);
    pdf.setTextColor(120, 120, 120);
    pdf.text('Generated by Quality Pumps — quality-pumps.com', 20, 280);
    pdf.text('Sign up free at quality-pumps.com to generate your own reports.', 20, 287);

    pdf.save('pump-curve-example-report.pdf');
  };

  // Export to PDF function
  const exportToPDF = () => {
    if (!user) {
      setShowGuestSignupDialog(true);
      return;
    }
    const pdf = new jsPDF();
        ...
        (() => {
          ...
          return (
            <PumpReport
              ...
              isGuest={!user}
              onSignUpClick={() => setShowGuestSignupDialog(true)}
            />
          );
        })()}

      <GuestSignupDialog
        open={showGuestSignupDialog}
        onClose={() => setShowGuestSignupDialog(false)}
        onDownloadExample={downloadExamplePDF}
      />
    </div>
  );
}
