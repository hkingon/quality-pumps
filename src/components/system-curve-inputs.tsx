...
export function SystemCurveInputs({
  ...
  const [tempName, setTempName] = useState(
    system.name || `System Curve ${index + 1}`
  );
  ...
  const handleNameCancel = () => {
    setTempName(system.name || `System Curve ${index + 1}`);
    setIsEditingName(false);
  };

  const displayName = system.name || `System Curve ${index + 1}`;
    ...
    <Card className='mb-4 border p-4'>
      ...
      <div className='space-y-3'>
        <div className='grid gap-1.5'>
          <Label htmlFor={`staticHead-${system.id}`}>Static Head:</Label>
          <Input
        ...
        <div className='grid gap-1.5'>
          <Label htmlFor={`operatingHead-${system.id}`}>Operating Head:</Label>
          <Input
            ...
            placeholder='Enter operating head'
            step='any'
          />
        </div>
